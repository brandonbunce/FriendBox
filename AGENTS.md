...
- **.fbox v4** files are the canonical format. 512-byte header (with 256-byte description field), frame size table, per-frame chunks of `[type 'I'/'P'/'S'][N-byte IMA ADPCM block][video RLE]`, optional trailing keyframe offset table, CRC32 over [64..EOF]. Audio is **interleaved per frame** (one ADPCM block per video frame, ~1225 samples at 22050 Hz / 18 fps) so it streams from any source at any file size — no PSRAM-full requirement. Skip-frame marker ('S') for held frames carries audio but no video bytes. Spec in [docs/design-docs/fbox-codec.md](docs/design-docs/fbox-codec.md); encoder is `FriendBox-Server/fbox.py`. v3 files are not readable — re-encode via `migrate_v4.py` on the server.
- **Playback fps target is 18.** `FBOX_MAX_FPS = 18` ([include/io.hpp](include/io.hpp)) — files with `fps > 18` are rejected at header parse. The cap exists because the LT7680 SPI bus must run at **40 MHz on this PCB**, not the 80 MHz the chip's datasheet allows in theory: 80 MHz produces a persistent SDRAM-corruption symptom (32-px right shift + stale rows at top, sticky until reboot) that we deleted all "recovery" code for. 24 fps is unachievable at the 40 MHz ceiling because the per-frame SPI burst (~46 ms for 230 KB) leaves no headroom for decode + pacing. The 40 MHz limit is enforced in [include/LGFX_ESP32_PCBA5981_GT911.hpp](include/LGFX_ESP32_PCBA5981_GT911.hpp) — do not raise it.
